        ...
        <div class="content-l -size-scale-tablet content-right">
          <div data-view="productList">
                        ...
                        <div class="item-thumbnail-actions -width-full">
                          <ul class="item-thumbnail-actions__list">
                            <li class="item-thumbnail-actions__control -icon-cart -responsive-size-l">
                              <span>
                                <a href="{{ url('addcart/'.$row_product->id.'-'.$row_product->alias.'.html') }}" class="js-google-analytics__list-add-to-cart" data-bind="item-cart-status:12897637" data-view="modalAjax" rel="nofollow" title="Add to Cart"><span>Add to Cart</span></a>
                                </span>
            ...
            <!-- <ul class="pagination__list">
              <li><a href="Ghost-Themes-s93-page-1"><img src="{{url('images/page_first.gif')}}"></a></li>
              @for($i = 1; $i <= $data->lastPage(); $i++)
              <li class="{{ $data->currentPage() == $i ? 'phantrang1' : '' }}"><a href="{{$data->url($i)}}">{{$i}}</a></li>
              @endfor
              
              <li><a href="Ghost-Themes-s93-page-1"><img src="{{url('images/page_last.gif')}}"></a></li>
            </ul> -->
            @if($data->lastPage()>=1)
            <div class="center">
              <div class="pagination">
                @if($data->currentPage() != 1)
                <a href="{{$data->url(1)}}">&laquo;</a>
                @endif
                ...
                <a href="{{$data->url($data->lastPage())}}">&raquo;</a>
                @endif
              </div>
            </div>
            @else
              <div class="center">
                <h3>Tạm thời chúng tôi chưa có sản phẩm này</h3>
              </div>
            @endif

        </div>

<!-- <li class="phantrang1"><span>7</span></li> --> 
